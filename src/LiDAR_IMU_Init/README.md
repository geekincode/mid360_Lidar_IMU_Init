...
- Since **LI-Init** supports Livox LiDAR, `livox_ros_driver2` must be built and sourced before launching LI-Init.
- Add `source $your_ws/install/setup.bash` into `~/.bashrc` (replace `$your_ws` with your ROS2 workspace path).
...
cd ..
colcon build --packages-select livox_ros_driver2 lidar_imu_init
...
After initialization and refinement finished, the result would be written into `src/LiDAR_IMU_Init/result/Initialization_result.txt`
