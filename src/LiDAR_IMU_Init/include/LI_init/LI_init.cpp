...
static inline double li_stamp_to_sec(const builtin_interfaces::msg::Time &stamp);
...
void LI_Init::push_ALL_IMU_CalibState(const sensor_msgs::msg::Imu::ConstSharedPtr &msg, const double &mean_acc_norm) {
    ...
    IMUstate.linear_acc =
            ...
            G_m_s2;
    IMUstate.timeStamp = li_stamp_to_sec(msg->header.stamp);
...
void LI_Init::solve_Rotation_only() {
    ...
    problem_rot.SetManifold(R_LI_quat, quatParam);
...
void LI_Init::solve_Rot_bias_gyro(double &timediff_imu_wrt_lidar) {
    ...
    problem_ang_vel.SetManifold(R_LI_quat, quatParam);
...
void LI_Init::solve_trans_biasacc_grav() {
    ...
    problem_acc.SetManifold(R_GL0_quat, quatParam);
