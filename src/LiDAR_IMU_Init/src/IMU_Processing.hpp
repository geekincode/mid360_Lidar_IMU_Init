...
static inline double imu_stamp_to_sec(const builtin_interfaces::msg::Time &stamp) {
...
{
  RCLCPP_WARN(rclcpp::get_logger("lidar_imu_init.imu_processing"), "Reset ImuProcess");
  mean_acc      = V3D(0, 0, -1.0);
  mean_gyr      = V3D(0, 0, 0);
  angvel_last       = Zero3d;
  imu_need_init_    = true;
...
void ImuProcess::IMU_init(const MeasureGroup &meas, StatesGroup &state_inout, int &N)
{
  ...
  if (N % 20 == 1 || N <= 3)  // 每20个打印1次，或前3个都打印
  {
    RCLCPP_INFO(rclcpp::get_logger("lidar_imu_init.imu_processing"), 
      "IMU Initializing: %.1f %% (N=%d, mean_acc_norm so far: %.4f)", 
      double(N) / MAX_INI_COUNT * 100, N, 
      (N > 1) ? mean_acc.norm() : 0.0);
  }
  V3D cur_acc, cur_gyr;
...
void ImuProcess::propagation_and_undist(const MeasureGroup &meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  double imu_end_time = imu_stamp_to_sec(v_imu.back()->header.stamp);
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    if (imu_stamp_to_sec(tail->header.stamp) < last_lidar_end_time_)    continue;
    ...
    acc_avr   <<0.5 * (head->linear_acceleration.x + tail->linear_acceleration.x),
      ...
      fout_imu << setw(10) << imu_stamp_to_sec(head->header.stamp) << "  " << angvel_now.transpose()<< " " << acc_now.transpose() << endl;

    angvel_avr -= state_inout.bias_g;
    acc_avr     = acc_avr / IMU_mean_acc_norm * G_m_s2 - state_inout.bias_a;

    if(imu_stamp_to_sec(head->header.stamp) < last_lidar_end_time_)
        dt = imu_stamp_to_sec(tail->header.stamp) - last_lidar_end_time_;
    else
        dt = imu_stamp_to_sec(tail->header.stamp) - imu_stamp_to_sec(head->header.stamp);
    ...
    acc_s_last  = acc_imu;
    double &&offs_t = imu_stamp_to_sec(tail->header.stamp) - pcl_beg_time;
    ...
    if (imu_need_init_)
    {
        if(!LI_init_done){
            ...
            if (init_iter_num > MAX_INI_COUNT)
            {
                ...
                double measured_acc_norm = mean_acc.norm();
                RCLCPP_INFO(rclcpp::get_logger("lidar_imu_init.imu_processing"), 
                    "IMU Initialization Done: Gravity: %.4f %.4f %.4f, Acc norm: %.4f (config: %.4f, diff: %.4f)",
                    stat.gravity[0], stat.gravity[1], stat.gravity[2], 
                    measured_acc_norm, IMU_mean_acc_norm, measured_acc_norm - IMU_mean_acc_norm);
                IMU_mean_acc_norm = measured_acc_norm;
            }
        }
        else{
            cout << endl;
